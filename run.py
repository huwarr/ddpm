...
logging.info('Start training, epochs: {}...'.format(args.n_epochs))
...
logging.info('Loss on training set: {}'.format(losses[-1]))

# Generate samples
samples, steps = sample_func(ddpm, n_samples=args.n_samples, use_wandb=args.use_wandb)

# Save samples
samples_PIL = [Image.fromarray(sample[i].squeeze().numpy()) for sample in samples]
rows = 2
...
grid.save("generated samples.jpg")
